#!/usr/bin/env python3
"""nightly_improvement.py - Daily self-improvement loop (v5.6).

Authorized by Council session v5.6 (2026-04-24). Updated 2026-04-28 to load
sources from manifest/nightly_sources.json and run a web search pass (Step 1b).

Every scheduled fire (default: 16:03 local daily):
1.  Load source manifest from manifest/nightly_sources.json.
1a. Fetch all URLs in sources (official_docs + mcp_ecosystem + community etc).
1b. Run web search pass via DuckDuckGo for all search_queries (deduped by URL hash).
2.  Diff SHA256 hashes vs memory/nightly_doc_hashes.json.
3.  Extract signals by category (HOOKS/AGENTS/SKILLS/TOOLS/MEMORY/SECURITY/…).
4.  Generate proposal -> .claude/notes/concepts/proposals/nightly-<YYYY-MM-DD>.md.
5.  Submit protocol envelopes to all 5 Council members -> observability/traces.json.
6.  Tier 1-2: auto-apply + log. Tier 3+: PENDING_COUNCIL_DECISION.
7.  Summary -> memory/logs.json with outcome=nightly_improvement_run.
...
MANIFEST_PATH = REPO / "manifest" / "nightly_sources.json"

# Nightly frequency source groups (fetched every run)
_NIGHTLY_GROUPS = {"official_docs", "mcp_ecosystem"}
...
def load_manifest() -> dict:
    """Load nightly_sources.json; return empty manifest on failure."""
    try:
...
def get_nightly_urls(manifest: dict) -> list[str]:
    """Return all URLs scheduled for nightly fetch."""
    sources = manifest.get("sources", {})
    urls: list[str] = []
    for group, group_urls in sources.items():
        freq = (
            manifest.get("fetch_config", {})
            .get("scrape_frequency", {})
            .get(group, "weekly")
        )
        if freq == "nightly":
            urls.extend(group_urls)
    return urls or _FALLBACK_DOC_URLS


def get_search_queries(manifest: dict) -> list[str]:
    """Return search queries from manifest, falling back to empty list."""
    return manifest.get("search_queries", {}).get("queries", [])
...
def diff_docs(urls: list[str]) -> dict:
    ...
    return {"changed": changed, "errors": errors, "total_fetched": len(current), "total_urls": len(urls)}
...
def search_pass(queries: list[str] | None = None) -> dict:
    """Run search queries via DuckDuckGo, fetch unseen URLs, return findings dict."""
    if queries is None:
        queries = []
...
def main() -> int:
    ...
    # Load manifest — Step 1a sources + Step 1b search queries
    manifest = load_manifest()
    doc_urls = get_nightly_urls(manifest)
    queries = get_search_queries(manifest)
    print(f"[nightly] manifest loaded: {len(doc_urls)} nightly URLs, {len(queries)} search queries")

    # Step 1a — static URL fetch
    result = diff_docs(doc_urls)
    changed = result["changed"]
    errors = result["errors"]

    print(f"[nightly] fetched: {result['total_fetched']}/{result['total_urls']} docs")
    if errors:
        print(f"[nightly] errors fetching: {len(errors)} (URLs unreachable)")
    print(f"[nightly] changes detected: {len(changed)}")

    # Step 1b — search pass
    sr = search_pass(queries)
